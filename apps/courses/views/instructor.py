...
class InstructorRequiredMixin(LoginRequiredMixin, UserPassesTestMixin):
    def test_func(self):
        return self.request.user.is_superuser or self.request.user.is_instructor # type: ignore
...
class ModuleListView(InstructorRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return self.course.modules.all().order_by('order') # type: ignore
...
class ModuleCreateView(InstructorRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse('instructor:module_list',
                       args=[self.object.course.id]) # type: ignore
...
class ModuleDeleteView(InstructorRequiredMixin, DeleteView):
    ...
    def get_success_url(self):
        return reverse('instructor:module_list', args=[self.object.course.id]) # type: ignore
...
class ContentListView(InstructorRequiredMixin, ListView):
    ...
    def get_queryset(self):
        self.module = get_object_or_404(
            Module,
            id=self.kwargs['module_id'], course__owner=self.request.user)
        return self.module.contents.all().select_related('content_type').order_by('order') # type: ignore
...
class ContentCreateUpdateView(InstructorRequiredMixin, View):
    ...
    def get_form(self, model, *args, **kwargs):
        Form = modelform_factory(
            model, exclude=['owner', 'created_at',
                            'updated_at'])
        return Form(*args, **kwargs)

    def dispatch(self, request,
                 module_id=None, model_name=None, id=None, *args, **kwargs):
        self.module = get_object_or_404(
            Module, id=module_id,
        ...
        if id:
            try:
                content = Content.objects.select_related('content_type').get(
                    object_id=id,
                    content_type=ContentType.objects.get_for_model(self.model), # type: ignore
                    module=self.module
                )
        ...
        return super().dispatch(request,
                                module_id, model_name, id, *args, **kwargs)

    def get(self, request,
            module_id, model_name, id=None):
    ...
    def post(self, request, module_id, model_name, id=None):
        ...
        if form.is_valid():
            ...
            return redirect('instructor:content_list', module_id=self.module.id) # type: ignore
...
class ContentDeleteView(InstructorRequiredMixin, DeleteView):
    ...
    def get_success_url(self):
        return reverse('instructor:content_list',
                       args=[self.object.module.id]) # type: ignore
        ...
        try:
            ...
            for index, module_id in enumerate(order):
                modules = Module.objects.filter(
                    id=module_id,
                    course__owner=request.user).update(order=index)
